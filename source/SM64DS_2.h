...
extern "C"
{
	...
	void FUN_0202aaec(int,int,int,int,int);
	void FUN_0202d690(int param_1,int levelID);
	
	
	void FUN_0202ded4(int);
	
	extern char LEVEL_ID_TO_LOAD;
	extern char STAR_ID_SELECTED;
	extern char PREVIOUS_LEVEL_ID;
	extern char RETURN_ENTRANCE_ON_DEAH;
	extern char * DAT_0209f340;
	extern char* RAM_ADDRESS_LEVELS[52];
	
	extern int CURRENT_LEVEL_OVERLAY;
	extern int DAT_0209f278;
	void OS_Terminate(void);
	void FS_loadOverlay(int overlayID);
	void FUN_0202df70(void (*)(int),int param_2);
	int FS_LoadOverlayInfo(int* param_1,int param_2,int overlayID);
	
	
	int FS_Internal_readFile(int, int, int);
}
